...
const authPath = path.join(__dirname, 'backend/src/routes/auth.ts');
...
content = content.replace(/\{ userId: user\.id, role: user\.role \}/g, '{ userId: user.id, modules: loginModules || regModules || ALL_MODULES }');

// We need to rewrite login, register, and me manually to avoid mess.
fs.writeFileSync('rewrite_auth.js', `
const fs = require('fs');
